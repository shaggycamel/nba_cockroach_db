...
print(
    '\nWriting to database:',
    'cockroach' if 'cockroach' in str(dh.db_con.url) else 'postgre',
    '\n\n',
)
...
batch_attempt = read_sql_query(
    f"SELECT MAX(batch_attempt) FROM util.update_log WHERE process_date::DATE = '{nz_date}'",
    dh.db_con,
)['max']
...
if 'alt_freq_objs' in locals():
    update_schedule = read_sql_query(
        f"SELECT * FROM util.update_schedule WHERE table_name IN ('{alt_freq_objs}')", dh.db_con
    )
else:
    update_schedule = read_sql_query(
        'SELECT * FROM util.update_schedule WHERE pause IS FALSE ORDER BY table_name DESC',
        dh.db_con,
    )
...
#################################### Log end of process
failed_objects = read_sql_query(
    f"SELECT table_name, error_message FROM util.update_log WHERE successful_run = 'false' AND process_date::DATE = '{nz_date}' AND batch_attempt = {batch_attempt}",
    dh.db_con,
)
...
if len(failed_objects) > 0:
    ...
    for _, row in failed_objects.iterrows():
