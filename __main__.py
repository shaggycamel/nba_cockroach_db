from sys import argv
from polars import DataFrame
from datetime import datetime
from zoneinfo import ZoneInfo
from smtplib import SMTP
from sports_hub import SportsHub

hub = SportsHub(db_con = 'postgres')
# hub = SportsHub(db_con = 'cockroach')

print(
    '\nWriting to database:',
    'cockroach' if 'cockroach' in str(hub.db.engine.url) else 'postgres',
    '\n\n',
)

################################## Custom function to handle running & logging events
def custom_prelog(eval_string, table_name, batch_attempt):
    try:
        eval(eval_string)
        success = True
        error_message = None
    except Exception as e:
        success = False
        error_message = str(e)
        print('--------------------- NOT UPDATED:', table_name, '\n\n')
    finally:
        df_inr = DataFrame(
            {
                'table_name': table_name,
                'process_date': datetime.now(ZoneInfo('Pacific/Auckland')),
                'batch_attempt': batch_attempt,
                'successful_run': success,
                'error_message': error_message,
            }
        )
        hub.db.write(df_inr, 'update_log', 'util')



################################### Log start of process
nz_date = datetime.now(ZoneInfo('Pacific/Auckland')).strftime('%Y-%m-%d')

batch_attempt = hub.db.read(f"SELECT MAX(batch_attempt) FROM util.update_log WHERE process_date::DATE = '{nz_date}'")['max']
batch_attempt = 1 if batch_attempt[0] is None else batch_attempt[0] + 1

df_start = DataFrame(
    {
        'table_name': 'process start',
        'process_date': datetime.now(ZoneInfo('Pacific/Auckland')),
        'batch_attempt': batch_attempt,
        'successful_run': True,
        'error_message': None,
    }
)
hub.db.write(df_start, 'update_log', 'util')


#################################### Obtain update_schedule filtering on US Eastern Time
#################################### Loop through update schedule and update objects accordingly

# Used to be used in SQL query
us_eastern_time = datetime.now(ZoneInfo('America/New_York')).strftime('%Y-%m-%d')


try:
    # this determines if code was run interactively
    if argv[1] != '-f':
        alt_freq_objs = argv[1].replace(',', "','")
except IndexError as e:
    pass

if 'alt_freq_objs' in locals():
    update_schedule = hub.db.read(f"SELECT * FROM util.update_schedule WHERE table_name IN ('{alt_freq_objs}')")
else:
    update_schedule = hub.db.read('SELECT * FROM util.update_schedule WHERE pause IS FALSE ORDER BY table_name DESC')

# Iterate over rows and execute statments
for _, row in update_schedule.iter_rows():
    eval_string = ''.join(['hub.', row['associated_function'], '()'])
    custom_prelog(eval_string, row['table_name'], batch_attempt)


#################################### Log end of process
failed_objects = hub.db.read(f"SELECT table_name, error_message FROM util.update_log WHERE successful_run = 'false' AND process_date::DATE = '{nz_date}' AND batch_attempt = {batch_attempt}")

df = DataFrame(
    {
        'table_name': 'process end',
        'process_date': datetime.now(ZoneInfo('Pacific/Auckland')),
        'batch_attempt': batch_attempt,
        'successful_run': False if len(failed_objects) > 0 else True,
        'error_message': None,
    }
)
hub.db.write(df, 'update_log', 'util')


if len(failed_objects) > 0:
    email_address = 'oliverf.eaton@gmail.com'
    password = 'qckbndgopzwjkaxq'
    message = ','.join(failed_objects['table_name'].to_list()) + '\n\n'
    for _, row in failed_objects.iter_rows():
        message += row['table_name'] + '\n' + row['error_message'] + '\n\n'

    server = SMTP('smtp.gmail.com', 587)
    server.starttls()
    server.login(email_address, password)
    server.sendmail(email_address, email_address, f'Subject: nba-data-mgmt\n\n{message}')
    raise Exception(
        print(nz_date, '\nFailed objects:', failed_objects['table_name'].to_list(), '\n')
    )

else:
    print(nz_date, '\nAll tables successfully updated.')
